...
const GeminiOAuthModal = ({ visible, onCancel, onSuccess }) => {
  ...
  const startOAuth = async () => {
    if (!clientId.trim()) {
      showError(t('请填写 OAuth Client ID'));
      return;
    }
    ...
    try {
      const res = await API.post(
        '/api/channel/gemini/oauth/start',
        {
      ...
      if (!res?.data?.success) {
        console.error('Gemini OAuth start failed:', res?.data?.message);
        throw new Error(res?.data?.message || t('启动授权失败'));
      }
      const url = res?.data?.data?.authorize_url || '';
      if (!url) {
        console.error(
          'Gemini OAuth start response missing authorize_url:',
          res?.data,
        );
  ...
  const completeOAuth = async () => {
    ...
    if (!projectId.trim()) {
      showError(t('请填写 Google Cloud Project ID'));
      return;
    }
    ...
    try {
      const res = await API.post(
        '/api/channel/gemini/oauth/complete',
        {
          input: input.trim(),
          client_id: clientId.trim(),
      ...
      if (!res?.data?.success) {
        console.error('Gemini OAuth complete failed:', res?.data?.message);
        throw new Error(res?.data?.message || t('授权失败'));
      }

      const key = res?.data?.data?.key || '';
      if (!key) {
        console.error('Gemini OAuth complete response missing key:', res?.data);
    ...
    >
      <Space vertical spacing='tight' style={{ width: '100%' }}>
        <Banner
          type='info'
          description={t(
            '1) 填写 OAuth Client ID、Client Secret 和 Google Cloud Project ID；2) 点击「打开授权页面」完成登录；3) 浏览器会跳转到 localhost（页面打不开也没关系）；4) 复制地址栏完整 URL 粘贴到下方；5) 点击「生成并填入」。',
          )}
        />
        ...
        <Input
          ...
          prefix={t('Google Cloud Project ID')}
          showClear
        />
        <Input
          value={redirectUri}
        ...
        <Text type='tertiary' size='small'>
          {t(
            'Google Cloud Project ID 用于 x-goog-user-project，请使用已启用 Gemini API 的项目 ID。',
          )}
        </Text>
        ...
        <Text type='tertiary' size='small'>
          {t(
            '说明：生成结果是可直接粘贴到渠道密钥里的 JSON（包含 access_token / refresh_token / project_id）。',
          )}
        </Text>
      </Space>
    </Modal>
