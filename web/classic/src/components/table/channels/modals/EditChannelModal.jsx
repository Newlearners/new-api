...
import GeminiOAuthModal from './GeminiOAuthModal';
...
const getCodexCredentialValue = (credential, keys) => {
...
const hasCodexCredentialToken = (credential) => {
  const accessToken = getCodexCredentialValue(credential, [
    'access_token',
    'accessToken',
  ]);
  const sessionToken = getCodexCredentialValue(credential, [
    'session_token',
    'sessionToken',
  ]);
  ...
  const nestedCredential =
    credential?.openaiOauth || credential?.openai_oauth || null;
  return Boolean(
    getCodexCredentialValue(nestedCredential, [
      'access_token',
      'accessToken',
    ]) ||
    getCodexCredentialValue(nestedCredential, [
...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 57:
      return '请输入 JSON 格式的 OAuth 凭据，支持 access_token、session_token、accessToken 或 openaiOauth';
    default:
      return '请输入渠道对应的鉴权密钥';
  }
}
...
const EditChannelModal = (props) => {
  ...
  const originInputs = {
    ...
    // 仅 Gemini: 密钥格式（存入 settings.gemini_key_type）
  ...
  const [codexCredentialRefreshing, setCodexCredentialRefreshing] =
    useState(false);
  const [geminiOAuthModalVisible, setGeminiOAuthModalVisible] = useState(false);
  const [geminiCredentialRefreshing, setGeminiCredentialRefreshing] =
    useState(false);
    ...
    if (success) {
      ...
      if (data.settings) {
        try {
          ...
          data.gemini_key_type = parsedSettings.gemini_key_type || 'api_key';
          // 读取 AWS 密钥格式和区域
  ...
  const handleGeminiOAuthGenerated = (key) => {
    handleChannelOtherSettingsChange('gemini_key_type', 'oauth');
    try {
      handleInputChange('key', JSON.stringify(JSON.parse(key), null, 2));
    } catch (error) {
      handleInputChange('key', key);
    }
  };
  ...
  const handleRefreshGeminiCredential = async () => {
    if (!isEdit) return;

    setGeminiCredentialRefreshing(true);
    try {
      const res = await API.post(
      ...
      setGeminiCredentialRefreshing(false);
    }
  };
  ...
  const submit = async () => {
    ...
    if (localInputs.type === 57) {
      ...
      if (rawKey !== '') {
        ...
        try {
          ...
          if (!hasCodexCredentialToken(parsed)) {
            showInfo(
              t(
                '密钥 JSON 必须包含 access_token、session_token、accessToken、sessionToken 或 openaiOauth',
              ),
            );
            return;
          }
    ...
    if (localInputs.type === 24 && localInputs.gemini_key_type === 'oauth') {
      if (batch) {
        showInfo(t('Gemini OAuth 渠道不支持批量创建'));
        return;
      }
      ...
      if (rawKey !== '') {
        ...
        try {
          ...
          if (!accessToken && !refreshToken) {
            showInfo(
              t('Gemini OAuth JSON 必须包含 access_token 或 refresh_token'),
            );
            return;
          }
          if (!projectId) {
            showInfo(
              t('Gemini OAuth JSON 必须包含 project_id 或 quota_project_id'),
            );
            return;
          }
          localInputs.key = JSON.stringify(parsed);
    ...
    // type === 24 (Gemini): 保存密钥类型，api_key 或 oauth
    if (localInputs.type === 24) {
      settings.gemini_key_type = localInputs.gemini_key_type || 'api_key';
    } else if ('gemini_key_type' in settings) {
      delete settings.gemini_key_type;
    }
    ...
    // 顶层的 gemini_key_type 不应发送给后端
    delete localInputs.gemini_key_type;
  ...
  const isGeminiOAuthMode =
    inputs.type === 24 && inputs.gemini_key_type === 'oauth';
  const batchAllowed =
    (!isEdit || isMultiKeyChannel) && inputs.type !== 57 && !isGeminiOAuthMode;
                ...
                <Spin spinning={loading}>
                  <div className='p-2 space-y-3' ref={formContainerRef}>
                    ...
                    <Card className='!rounded-2xl shadow-sm border-0'>
                      ...
                      {inputs.type === 24 && (
                        <Form.Select
                          field='gemini_key_type'
                          label={t('Gemini 密钥格式')}
                          placeholder={t('请选择密钥格式')}
                          optionList={[
                            { label: 'API Key', value: 'api_key' },
                            { label: 'Google OAuth JSON', value: 'oauth' },
                          ]}
                          style={{ width: '100%' }}
                          value={inputs.gemini_key_type || 'api_key'}
                          onChange={(value) => {
                            handleChannelOtherSettingsChange(
                              'gemini_key_type',
                              value,
                          ...
                          extraText={
                            (inputs.gemini_key_type || 'api_key') === 'oauth'
                              ? t(
                                  'OAuth 模式使用 Google 账号 refresh_token，不支持批量创建',
                                )
                              : t('API Key 模式使用普通 Google Gemini API Key')
                          }
                        />
                      )}
                          ...
                          {inputs.type === 57 ? (
                            <>
                              <Form.TextArea
                                ...
                                placeholder={t(
                                  '请输入 JSON 格式的 OAuth 凭据，例如：\n{\n  "access_token": "...",\n  "refresh_token": "...",\n  "account_id": "..."\n}\n也支持仅包含 session_token，或包含 accessToken / refreshToken / openaiOauth 的导出格式。',
                                )}
                                rules={
                                  isEdit
                                    ? []
                                ...
                                extraText={
                                  <div className='flex flex-col gap-2'>
                                    <Text type='tertiary' size='small'>
                                      {t(
                                        '仅支持 JSON 对象，必须包含 access_token、session_token、accessToken、sessionToken 或 openaiOauth',
                                      )}
                                    </Text>
                            ...
                            (inputs.gemini_key_type || 'api_key') ===
                              'oauth' ? (
                            <>
                              <Form.TextArea
                                field='key'
                                ...
                                placeholder={t(
                                  '请输入 Gemini OAuth JSON 凭据，或点击下方 Gemini 授权生成',
                                )}
                                rules={
                                  isEdit
                                    ? []
                                ...
                                extraText={
                                  <div className='flex flex-col gap-2'>
                                    <Text type='tertiary' size='small'>
                                      {t(
                                        'Gemini OAuth JSON 必须包含 access_token 或 refresh_token，并包含 project_id 或 quota_project_id',
                                      )}
                                    </Text>
                                    ...
                                    <Space wrap spacing='tight'>
                                      <Button
                                        ...
                                        theme='outline'
                                        onClick={() =>
                                          setGeminiOAuthModalVisible(true)
                                        }
                                        disabled={isIonetLocked}
                                      >
                                        {t('Gemini 授权')}
                                      ...
                                      {isEdit && (
                                        <Button
                                          ...
                                          theme='outline'
                                          onClick={
                                            handleRefreshGeminiCredential
                                          }
                                          loading={geminiCredentialRefreshing}
                                          disabled={isIonetLocked}
                                        >
                                          {t('刷新凭证')}
                                        </Button>
                              ...
                              <GeminiOAuthModal
                                visible={geminiOAuthModalVisible}
                                onCancel={() =>
                                  setGeminiOAuthModalVisible(false)
                                }
                                onSuccess={handleGeminiOAuthGenerated}
                              />
                            </>
                          ) : inputs.type === 41 &&
                            (inputs.vertex_key_type || 'json') === 'json' ? (
